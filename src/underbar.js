...
//       var val = (iterator !== undefined ? iterator(array[i]) : array[i]);
//       if (!mySet.has(val)) {
//         result.push(array[i]);
//         mySet.add(val);
//       }
...
//     return result;
    ...
    if(iterator === undefined) {
      ...
      for(let j = 0; j < array.length; j++) {
        booleanArr.push(iterator(array[j]))
      }
      for(let k = 0; k < booleanArr.length; k++) {
        if(!result.includes(booleanArr[k])) {
          result.push(booleanArr[k])
          returnArr.push(array[k])
        }
      }
    }
    return returnArr
  ...
  _.reduce = function(collection, iterator, accumulator) {
    ...
    if(accumulator === undefined) {
      ...
      for(let i = 1; i < collection.length; i++) {
        accu = iterator(accu, collection[i])
      }
      return accu
    } else {
  ...
  _.shuffle = function(array) { 
    ...
    if (randomNumber - 1 > 0) {
      var temp = copy[randomNumber]
      copy[randomNumber] = copy[randomNumber-1]
      copy[randomNumber-1] = temp;
      return copy
    } else {
      var temp = copy[0];
